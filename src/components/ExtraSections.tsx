...
export const ProcessSteps = () => {
  ...
  return (
    <section id="method" className="py-24 lg:py-40 bg-[#050505] border-y border-zinc-900 relative overflow-hidden">
      ...
      <div className="hidden md:block absolute top-0 left-1/4 w-[500px] h-[500px] bg-yellow-500/5 blur-[120px] rounded-full -z-10" />
      <div className="hidden md:block absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-yellow-500/5 blur-[120px] rounded-full -z-10" />
            ...
            >
              <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
                ...
                <div className="relative mb-12">
                  <div className="w-28 h-28 bg-zinc-900 border-2 border-zinc-800 rounded-[40px] flex items-center justify-center text-white md:group-hover:border-yellow-500/50 md:group-hover:shadow-[0_0_40px_rgba(234,179,8,0.15)] transition-all duration-500 relative z-10 overflow-hidden will-change-transform">
                    <div className="absolute inset-0 bg-gradient-to-br from-yellow-500/5 to-transparent opacity-0 md:group-hover:opacity-100 transition-opacity" />
                    <step.icon className="w-12 h-12 text-yellow-500 md:group-hover:scale-110 transition-transform duration-500" />
                  </div>
                  
                  {/* Floating Number */}
                  <div className="absolute -top-4 -right-4 w-12 h-12 bg-yellow-500 text-black rounded-[18px] flex items-center justify-center text-xl font-black italic shadow-2xl shadow-yellow-500/40 z-20 border-4 border-[#050505] md:group-hover:rotate-12 transition-transform duration-500 will-change-transform">
                    {i + 1}
                  </div>

                  {/* Decorative Glow - Desktop only */}
                  <div className="hidden md:block absolute inset-0 bg-yellow-500/20 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-10" />
                </div>
    ...
    <footer className="bg-black text-white py-16 lg:py-24 border-t border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-12 mb-16 lg:mb-20">
          <div>
            <div className="text-2xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-amber-500 mb-6 italic">
              APPZHUB
            </div>
